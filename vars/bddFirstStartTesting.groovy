...
@Field
BddFirstStartOptional stageOptional
 ...
 */
void call(PipelineConfiguration config, BddFirstStartOptional stageOptional, PipelineState state) {
  this.config = config
  this.state = state
  ...
  catchError(message: 'Ошибка во время архивации отчетов о тестировании', buildResult: 'FAILURE', stageResult: 'FAILURE') {
    testResultsHelper.packTestResults(config, stageOptional, state.bddFirstStart)
  }
...
private bddTesting(String credential = '') {
  def command = VRunner.vanessaFirstStart(config, stageOptional)
